...
select COUNT(driver_id) from participated p, accident a
...
select COUNT(distinct a.report_no) from accident a
where exists 
...
select distinct model, c_year from car c, participated p
...
select driver_name, address from person p, owns o
...
select driver_name from person p, accident a, participated ptd
